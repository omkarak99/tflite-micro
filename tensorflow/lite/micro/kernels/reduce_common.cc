...
TfLiteStatus PrepareProdHelper(TfLiteContext* context, TfLiteNode* node,
  ...
  if (input->type == kTfLiteInt8 || input->type == kTfLiteInt16) {
    //const double real_multiplier = static_cast<double>(input->params.scale) /
    //                               static_cast<double>(output->params.scale);
    int reduced_axis_size = NumElements(input) / NumElements(output);
    const double real_multiplier = static_cast<double>(input->params.scale) /
    			   static_cast<double>(std::pow(static_cast<double>(output->params.scale), 1.0 / reduced_axis_size));
    QuantizeMultiplier(real_multiplier, &op_data->multiplier, &op_data->shift);
  }
...
TfLiteStatus EvalProdHelper(TfLiteContext* context, TfLiteNode* node,
  ...
  int temp_index[kMaxNumberOfAxis];
  int resolved_axis[kMaxNumberOfReducedAxis];
  int reduced_axis_size = NumElements(input->dims) / NumElements(output->dims);
  const double real_multiplier = static_cast<double>(op_data->input_scale) /
  				   static_cast<double>(std::pow(static_cast<double>(op_data->output_scale), 1.0 / reduced_axis_size));
  switch (input->type) {
    case kTfLiteFloat32: {
      TF_LITE_ENSURE(
          context,
    ...
    case kTfLiteInt8: {
      QuantizeMultiplier(real_multiplier, &op_data->multiplier, &op_data->shift);
      int32_t* temp_prod = static_cast<int32_t*>(context->GetScratchBuffer(context, op_data->temp_buffer_idx));
      bool result = reference_ops::QuantizedReduceProd(tflite::micro::GetTensorData<int8_t>(input), op_data->input_zp, tflite::micro::GetTensorShape(input),
							tflite::micro::GetTensorData<int8_t>(output), op_data->output_zp, tflite::micro::GetTensorShape(output),
							tflite::micro::GetTensorData<int>(axis), op_data->num_axis,
							params->keep_dims, temp_index, resolved_axis, temp_prod, op_data->multiplier, op_data->shift);
      TF_LITE_ENSURE(context, result);
    ...
    case kTfLiteInt16: {
      QuantizeMultiplier(real_multiplier, &op_data->multiplier, &op_data->shift);
      int32_t* temp_prod = static_cast<int32_t*>(context->GetScratchBuffer(context, op_data->temp_buffer_idx));
      bool result = reference_ops::QuantizedReduceProd(tflite::micro::GetTensorData<int16_t>(input), op_data->input_zp, tflite::micro::GetTensorShape(input),
							tflite::micro::GetTensorData<int16_t>(output), op_data->output_zp, tflite::micro::GetTensorShape(output),
							tflite::micro::GetTensorData<int>(axis), op_data->num_axis,
							params->keep_dims, temp_index, resolved_axis, temp_prod, op_data->multiplier, op_data->shift);
      TF_LITE_ENSURE(context, result);
